...
function updateTotalAmount() {
  ...
  if (offers.length) {
    offers.forEach(o => {
      ...
      if (type === "topup") {
        const gold = o.dataset.gold;
        const p = priceLists[method].topup.find(x => x.gold.replace(/,/g, "") === gold);
        priceStr = p?.price || "";
        details += `• ${gold} جوهرة - ${priceStr}<br>`;
      } else {
  ...
  function recalcSelectedAmountUI() {
    ...
    selectedOffers.forEach(offer => {
      const key = offer.gold ? `${offer.gold}_gold` : offer.offerName;
      const price = offersPrices[key];

      if (price !== undefined) {
        total += price;
        breakdownLines.push(`${offer.gold ? offer.gold + " جوهرة" : offer.offerName} : ${price.toFixed(2)} د.أ`);
      } else {
        breakdownLines.push(`• ${offer.gold || offer.offerName}: ❓ غير معروف`);
      }
    });
...
(function () {
  ...
  async function fetchState() {
    ...
    try {
      const snap1 = await db.collection('topup').doc('bloodstrike').get();
      if (snap1.exists) {
    ...
    const snap2 = await db.collection('bloodstrike').doc('state').get();
    if (snap2.exists) {
      return snap2.data() || {};
    }
...
function applyConfig(cfg) {
  ...
  Object.entries(topupMap).forEach(([gold, status]) => {
  ...
  function listenStateRealtime() {
    ...
    // 1) topup/bloodstrike.state (JSON string)
    try {
      db.collection('topup').doc('bloodstrike').onSnapshot(snap => {
        ...
        if (typeof data.state === 'string') {
          try {
            ...
            console.warn('JSON غير صالح في topup/bloodstrike.state');
          }
        }
      });
    } catch (_) {}

    // 2) bloodstrike/state (object)
    try {
      db.collection('bloodstrike').doc('state').onSnapshot(snap => {
        if (snap.exists) {
          const cfg = snap.data() || {};
          applyConfig(cfg);
        }
