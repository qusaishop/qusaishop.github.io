...
async function loadPrices(useruid = null, { timeoutMs = 5000, silentOnCached = true } = {}) {
  ...
  try {
    ...
    if (!useruid) return;
    const url = new URL('https://8ball.qsystwr820.workers.dev/');
...
async function sendOrder() {
  ...
  let total, breakdown;
  try {
    const priceRes = await fetch("https://8ball.qsystwr820.workers.dev/", {
      method: "POST",
  ...
  try {
    ...
    const response = await fetch("https://8ball.qsystwr820.workers.dev/", {
      method: "POST",
      headers: {
