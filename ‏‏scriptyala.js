...
function updateTotalAmount() {
  ...
  function recalcSelectedAmountUI() {
    ...
    selectedOffers.forEach(offer => {
      ...
      if (price !== undefined) {
        total += price;
        breakdownLines.push(`${offer.jewels ? offer.jewels + " جوهرة" : offer.offerName} : ${price.toFixed(2)} د.أ`);
      } else {
...
(function () {
  ...
  async function fetchState() {
    ...
    try {
      const snap1 = await db.collection('topup').doc('yala').get();
      if (snap1.exists) {
    ...
    const snap2 = await db.collection('yala').doc('state').get();
    if (snap2.exists) {
      return snap2.data() || {};
    }
...
function applyConfig(cfg) {
  ...
  function listenStateRealtime() {
    ...
    // 1) topup/yala.state (JSON string)
    try {
      db.collection('topup').doc('yala').onSnapshot(snap => {
        ...
        if (typeof data.state === 'string') {
          try {
            ...
            console.warn('JSON غير صالح في topup/yala.state');
          }
        }
      });
    } catch (_) {}
    ...
    // 2) yala/state (object)
    try {
      db.collection('yala').doc('state').onSnapshot(snap => {
        if (snap.exists) {
          const cfg = snap.data() || {};
          applyConfig(cfg);
        }
